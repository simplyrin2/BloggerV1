...
class User(db.Model, UserMixin):
    ...
    posts = db.relationship('Post', uselist=True, backref='created_by', order_by='post.c.created_on.desc()', cascade='all, delete')
    comments = db.relationship('Comment', uselist=True, backref='commentor', cascade='all, delete')
    notifications = db.relationship('Notification', uselist=True, backref='user', primaryjoin='User.user_id==notification.c.to_notify', cascade='all, delete')
...
class Post(db.Model):
    ...
    likes = db.relationship('User', secondary='like', primaryjoin='Post.post_id==like.c.post_id', secondaryjoin='User.user_id==like.c.liked_by', uselist=True, backref='post')
